    ...
    <main className="container py-12">
      <div className="mx-auto max-w-2xl">
        <h1 className="text-3xl font-bold text-accent">
          Join Switch Smart Save
        </h1>
        <p className="mt-2 text-foreground/80">
          Complete the 3 quick steps. No deposit, instant eSIM after
          verification.
        </p>
        ...
        <div className="mt-6 grid gap-6 rounded-2xl border border-border bg-white p-6 shadow-sm">
          ...
          {step === 2 && (
            <section aria-labelledby="select-group">
              ...
              <p className="mt-1 text-sm text-foreground/80">
                Pick the community that fits you. You can change it later.
              </p>
                  ...
                  [
                    {
                      key: "student",
                      title: "Students",
                      desc: "Zero-deposit SIM + WiFi",
                    },
                    {
                      key: "professional",
                      title: "Young Pros",
                      desc: "Mobile + broadband + energy",
                    },
                    {
                      key: "family",
                      title: "Families",
                      desc: "Home bundle + green tariff",
                    },
              ...
              <div className="mt-6 flex items-center justify-between">
                <button
                  onClick={() => setStep(1)}
                  className="rounded-lg border border-border bg-white px-5 py-2"
                >
                  Back
                </button>
                <button
          ...
          {step === 3 && (
            <section aria-labelledby="verify-id">
              <h2 id="verify-id" className="text-xl font-semibold text-accent">
                Step 3: Verify ID
              </h2>
              <p className="mt-1 text-sm text-foreground/80">
                Upload a student card, proof of address or employee ID. We
                verify for eligibility and discounts.
              </p>
              <div className="mt-4">
                <input
                  type="file"
          ...
          {step === 4 && (
            <section aria-labelledby="confirmed">
              <h2 id="confirmed" className="text-xl font-semibold text-accent">
                Welcome aboard!
              </h2>
              <p className="mt-1 text-sm text-foreground/80">
                Your account is set. Use the QR/eSIM details below to activate
                your mobile plan.
              </p>
              <div className="mt-4 grid gap-4 sm:grid-cols-2">
                <div className="rounded-xl border border-border bg-muted/40 p-4">
                  ...
                  <p className="mt-1 select-all text-lg font-bold text-accent">
                    SSS-ESIM-
                    {Math.random().toString(36).slice(2, 8).toUpperCase()}
                  </p>
                  <p className="mt-2 text-xs text-foreground/60">
                    Scan in your phone settings to add a mobile plan.
                  </p>
                </div>
                <div className="rounded-xl border border-border bg-muted/40 p-4">
                  <p className="text-sm text-foreground/60">
                    QR for broadband setup
                  </p>
                  <div className="mt-2 grid grid-cols-5 gap-1">
                    {Array.from({ length: 25 }).map((_, i) => (
                      <span
                        key={i}
                        className={`block aspect-square ${i % 3 === 0 ? "bg-accent" : "bg-white"} border border-muted`}
                      />
                    ))}
                  </div>
                </div>
              ...
              <div className="mt-6 flex flex-wrap gap-3">
                <button
                  ...
                  className="rounded-lg bg-secondary px-5 py-2 text-white"
                >
                  Go to Portal
                </button>
                <button
        ...
        <p className="mt-4 text-xs text-foreground/60">
          We comply with GDPR. Data is encrypted in transit and at rest.
          Verification is used only to confirm eligibility and discounts.
        </p>
      </div>
    </main>
  );
    ...
    <div className="grid grid-cols-4 items-center gap-3">
      {[
        ...
        <div key={s.n} className="flex items-center gap-2">
          ...
          />
          <span
            className={`hidden text-xs sm:inline ${step >= s.n ? "text-accent" : "text-foreground/60"}`}
          >
            {s.label}
          </span>
        </div>
